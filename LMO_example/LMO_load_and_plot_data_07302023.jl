...
# your initial values and optimization flags
initial_parameters = Dict("Jab" => -0.2, "Jc" => 0.14, "D" => -0.04, "S" => 2.0)
initial_parameters = Dict("Jab" => -0.6, "Jc" => 0.3, "D" => -0.5, "S" => 2.0)
param_to_optimize = Dict("Jab" => true, "Jc" => true, "D" => true, "S" => false)

# Add parameter bounds. Here, parameters without specified bounds are assumed to have (-Inf, Inf) bounds.
param_bounds = Dict("Jab" => (-1.0, 0.0), "Jc" => (0.0, 1.0), "D" => (-1.0, 0.0))  # all values in tuples are either Float64 or Int

# Initialize Model with parameters, flags for optimization and parameter bounds
analytical_model = AnalyticalModel("analytical", initial_parameters, param_to_optimize, param_bounds, analytical_dispersion)

# Now, you don't need to pass `analytical_dispersion` to `refine_model!` and `update_dispersion_and_residuals!` functions
...
#LaMnO3
#https://journals.aps.org/prb/abstract/10.1103/PhysRevB.54.15149
...
print_symmetry_table(cryst, 6.0)

this_S = 2

sys = System(cryst, (1,1,1), [SpinInfo(1,S=this_S)], :dipole)
...
# Function to set the original spin configurations
...
swt = SpinWaveTheory(sys, energy_ϵ = 1e-5, energy_tol = 1e-6)
...
sunny_model = SunnyModel("Sunny", sunny_parameters, param_to_optimize, param_bounds, sunny_dispersion, sys, swt)

set_system_params!(sys, sunny_model.parameters.values)

print(energy(sys), '\n')

refine_model!(sunny_model, LMO_data, false)
...
objective_function_maps, param_ranges = objective_function_mapper(analytical_model, LMO_data, 50)
plot_all_heatmaps(objective_function_maps, param_ranges, analytical_model)

objective_function_maps, param_ranges = objective_function_mapper(analytical_model, LMO_data, 2.0, 50)
plot_all_heatmaps(objective_function_maps, param_ranges, analytical_model)