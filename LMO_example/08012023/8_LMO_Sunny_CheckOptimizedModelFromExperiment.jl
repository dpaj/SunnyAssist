...
function plot_all_heatmaps(heatmaps::Dict{Tuple{String, String}, Array{Float64, 2}}, param_ranges::Dict{String, AbstractRange}, model::AbstractModel)
    for ((param1, param2), heatmap_data) in heatmaps
        p = Plots.heatmap(param_ranges[param1], param_ranges[param2], heatmap_data,
        ...
        Plots.scatter!([optimal_params[param1]], [optimal_params[param2]], color=:red, markersize=10, label="Optimal solution")
...
@time begin
    objective_function_maps, param_ranges = objective_function_mapper(sunny_model, LMO_data, 20)
    plot_all_heatmaps(objective_function_maps, param_ranges, sunny_model)
end

@time begin
    objective_function_maps, param_ranges = objective_function_mapper(sunny_model, LMO_data, 0.1, 20)
    plot_all_heatmaps(objective_function_maps, param_ranges, sunny_model)
end


#=
function calculate_covariance(objective_function_map, param_ranges)
    minima = Dict{Tuple{String, String}, Tuple{CartesianIndex{2}, Float64}}()

    # Find minima for each heatmap
    for (param_key, heatmap) in objective_function_map
        min_index = argmin(heatmap)
        min_value = heatmap[min_index]
        minima[param_key] = (min_index, min_value)
    end

    # Select the heatmap with the global minimum
    global_min_key = argmin(minima) do (_, (_, val))
        return val
    end
    
    global_min_coords, _ = minima[global_min_key]

    # Calculate second derivatives numerically
    hessian = zeros(length(min_coords), length(min_coords))
    for i in 1:length(min_coords)
        for j in 1:length(min_coords)
            if i == j
                delta = param_ranges[i][2] - param_ranges[i][1]
                hessian[i, j] = (objective_function_map[min_coords .+ CartesianIndex(i => delta)] - 2 * min_value + objective_function_map[min_coords .+ CartesianIndex(i => -delta)]) / delta^2
            else
                delta_i = param_ranges[i][2] - param_ranges[i][1]
                delta_j = param_ranges[j][2] - param_ranges[j][1]
                hessian[i, j] = (objective_function_map[min_coords .+ CartesianIndex(i => delta_i, j => delta_j)] - objective_function_map[min_coords .+ CartesianIndex(i => delta_i, j => -delta_j)] - objective_function_map[min_coords .+ CartesianIndex(i => -delta_i, j => delta_j)] + objective_function_map[min_coords .+ CartesianIndex(i => -delta_i, j => -delta_j)]) / (4 * delta_i * delta_j)
            end
        end
    end
...
calculate_covariance(objective_function_maps, param_ranges)
